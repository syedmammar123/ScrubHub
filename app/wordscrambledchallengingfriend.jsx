...
export default function WordScrambledChallengingFriend() {
  //Question Fetch
  const { getChallengingFriendsQuestion } = useQuesStore((state) => state);

  const [question, setQuestion] = useState({ letterChoices: [], answer: "" });
  const [answerLength, setAnswerLength] = useState(
    getChallengingFriendsQuestion()?.answer?.replace(/\s/g, "").length
  );
  ...
  useEffect(() => {
    let q = {};
    if (getChallengingFriendsQuestion()?.questionStyle === "scrabble") {
